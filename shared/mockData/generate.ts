 ...
 */
let createMultipleGenerator: <T>(gen: FakeDataGenerator<T>) => (count: number) => T[];
...
const message: FakeDataGenerator<Message> = ({ id, content, user: defaultUser } = {}) => ({ id: id || genId(), content: content || faker.lorem.words(), user: defaultUser || user({}), __typename: 'Message' });

const messages = createMultipleGenerator(message);

const room: FakeDataGenerator<Room> = ({ id, name, messages: defaultMessages, users: defaultUsers } = {}) => ({ id: id || genId(), name: name || faker.internet.userName(), users: defaultUsers || users(4), messages: defaultMessages || messages(4), __typename: 'Room' });
